...
class TestBaseModel_instantiation(unittest.TestCase):
	...
	def test_str_representation(self):
    	...
    	bsm = BaseModel()
    	bsm.id = "123456"
    	bsm.created_at = bsm.updated_at = dt
    	bsmstr = bsm.__str__()
    	self.assertIn("[BaseModel] (123456)", bsmstr)
    	self.assertIn("'id': '123456'", bsmstr)
    	self.assertIn("'created_at': " + dt_repr, bsmstr)
    	self.assertIn("'updated_at': " + dt_repr, bsmstr)
	...
	def test_instantiation_with_kwargs(self):
    	dt = datetime.today()
    	dt_iso = dt.isoformat()
    	bsm = BaseModel(id="345", created_at=dt_iso, updated_at=dt_iso)
    	self.assertEqual(bsm.id, "345")
    	self.assertEqual(bsm.created_at, dt)
    	self.assertEqual(bsm.updated_at, dt)
	...
	def test_instantiation_with_args_and_kwargs(self):
    	dt = datetime.today()
    	dt_iso = dt.isoformat()
    	bsm = BaseModel("12", id="345", created_at=dt_iso, updated_at=dt_iso)
    	self.assertEqual(bsm.id, "345")
    	self.assertEqual(bsm.created_at, dt)
    	self.assertEqual(bsm.updated_at, dt)
